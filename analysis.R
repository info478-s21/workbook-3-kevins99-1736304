...
# Load prepped data
health <- read.csv("./data/prepped/all_data.csv")
...
data2016 <- health%>%
  filter(year == 2016)
ggplot(data2016) + geom_point(mapping = aes(x = le, y = hale)) +
  labs(title = "Life Expectancy vs HALE", x = "Life Expectancy", y = "HALE")
ggsave("charts/le_hale_graph.png")
cor(data2016$hale, data2016$le)
...
ggplot(data2016) + geom_point(mapping = aes(x = dalys, y = hale)) +
  labs(title = "DALYS vs HALE", x = "DALY", y = "HALE")
ggsave("charts/dalys_hale_graph.png")
cor(data2016$hale, data2016$dalys)
...
wide <- health%>%
  pivot_wider(names_from = year, values_from = c(hale, le, dalys))
...
ggplot(wide) + geom_point(mapping = aes(x = le_diff, y = hale_diff)) +
  labs(title = "Life Expectancy difference vs HALE difference", x = "Change in Life Expectancy", y = "Change in HALE") +
  geom_abline(intercept = 0, slope = 1) + xlim(-15, 20) + ylim(-15, 20)
ggsave("charts/change_graph.png")
# Interpretation: The dots correspond with the 45 degree line indicates that the change in life expectancy and
# the change in HALE have similar amount for the same country throughout the given time length.